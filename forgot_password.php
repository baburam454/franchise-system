...
{
    $query = "UPDATE registration SET verification_code = '$otp' WHERE email = '$email'";
...
{
    $query = "SELECT COUNT(*) FROM registration WHERE email = '$email' AND verification_code = '$otp'";
    $result = mysqli_query($connection, $query);
    $count = mysqli_fetch_row($result)[0];
    return $count > 0;
}
...
{
    $query = "UPDATE registration SET password = '$newPassword' WHERE email = '$email'";
    mysqli_query($connection, $query);
}
